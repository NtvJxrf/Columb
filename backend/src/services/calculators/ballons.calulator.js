const ballons = (data, selfcost) => {
    ...
    const result = {
        ...
        cut: {cost: (selfcost[`Закройщик, цена без налогов, мес`] / 22 / 6) / 60 * cuttingTime, count: cuttingTime},
        build: {cost: (selfcost[`Сборщик, цена без налогов, мес`] / 22 / 8) / 60 * buildTime, count: buildTime},
    }
    if(data.reinforcement != "none" && data.reinforcement != "fabric"){
        result.additional = {name: `Усиление привальным брусом ${data.reinforcement.toString().replace('.', ',')} см`, cost: selfcost[`Усиление 1 м.п, ширина ${data.reinforcement.toString().replace('.', ',')}см`], count: ((data.length - reinfMap[data.shape]) / 1000)}
    ...
    if (result.cut) {
        price += result.cut.cost;
    }
    if (result.build) {
        price += result.build.cost;
    }
    if (result.additional) {
        price += result.additional.cost * result.additional.count;
    }
    result.price = price
    return result
}
